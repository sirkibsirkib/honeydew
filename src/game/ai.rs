use crate::bit_set::BitIndex;
use crate::bit_set::FullBitIndexMap;
use crate::bit_set::INDICES;

use crate::{
    game::{rendering::VIEW_SIZE, Coord, PlayerColor, Room, World, PLAYER_SIZE, TOT_CELL_COUNT},
    prelude::*,
};
...
impl AiExt for QualityClimbAi {
    fn new(my_color: PlayerColor, rng: &mut Rng) -> Box<dyn Ai> {
        let prey_maybe_at = Coord::random(rng);
        Box::new(Self {
            my_color,
            sink_to_teleporters: FullBitIndexMap::new_copied(0),
            sink_to_prey: FullBitIndexMap::new_copied(0),
            sink_to_predator: FullBitIndexMap::new_copied(0),
            goal: prey_maybe_at,
            prey_maybe_at,
            predator_maybe_at: Coord::random(rng),
        })
    }
}
impl Ai for QualityClimbAi {
    fn i_was_moved(&mut self) {}
    fn update(&mut self, world: &World, rng: &mut Rng) -> Vel {
        let pos_centered = {
